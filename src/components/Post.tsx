...
const Post = async ({ type }: { type?: "status" | "comment" }) => {
  // FETCH POST MEDIA (comentat temporar)
  ...
  return (
    <div className="p-2 border-y-[1px] border-borderGray overflow-hidden max-w-2xl mx-auto">
      {/* POST TYPE */}
      ...
      {/* POST CONTENT */}
      <div className={`flex gap-4 ${type === "status" ? "flex-col" : "flex-row"} max-w-full md:flex-row`}>
        {/* AVATAR */}
        <div className={`${type === "status" && "hidden"} relative w-10 h-10 rounded-full overflow-hidden md:w-12 md:h-12`}>
          <ImageU path="general/avatar.png" alt="" w={100} h={100} tr={true} />
        </div>
        {/* CONTENT */}
        <div className="flex-1 flex flex-col gap-2">
          ...
          <Link href={`/lamaWebDev/status/123`}>
            <p className={`${type === "status" && "text-lg"} max-w-prose break-words`}>
          ...
          <ImageU path="general/post.jpeg" alt="" w={600} h={600} className="max-w-full h-auto" />
